...
public class PTP_ORDER_MACD_steps 
{
	...
	public PTP_ORDER_MACD_steps() throws FileNotFoundException, IOException
	{
		logger=LoggerGen.logGen(PTP_ORDER_MACD_steps.class.getName());
	...
	public void cnod_of_PTP_is_PUSHED_to_OCX_for_UNI_A_And_UNI_B(String eNv, String stateRegion) throws Throwable 
	{
		 logger.info("==========PTP pushed===========");
			...
			orderId=oaIds.get("productOrderItemId").get(2);
			 assignProductId=oaIds2.get("assignedProductId").get(2);
			 
			
	}

	@When("^complete the PTP order by NOTIFY on\"([^\"]*)\"$")
	public void complete_the_PTP_order_by_NOTIFY_on(String eNv) throws Exception 
	{
	...
	@When("^pass assignProductId from PTP response to MACD Request\"([^\"]*)\"$")
	public void pass_assignProductId_from_PTP_response_to_MACD_Request(String eNv) throws Throwable 
	{
